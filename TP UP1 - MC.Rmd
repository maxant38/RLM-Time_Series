...
Je teste les différentes améliorations suggérées précédmeent (transofmation ^2 , sqrt, log). Et je compare les modèles en calculant le RMSE.
```{r}
...
```
D'après les résultats, lm5.fit est notre meilleur modèle.
...
fitBest <- auto.arima(dataTrain[,2], D= 1, d=1, approximation = FALSE, seasonal = TRUE)
...
yEstime9 <- (yEstime4 + yEstime5)/2
residus9 <- yTest-yEstime9
compute_rmse(residus9)


yEstime10 <- (yEstime4 + yEstime5 + yEstime6)/3
residus10 <- yTest-yEstime10
compute_rmse(residus10)
