...
Je vais essayer de combiener les résultats de ces deux modèles.


```{r}


yEstimeFinal <- (yEstime3 + yEstime5 + yEstime7 )/3
...
```
On obtient une faible RMSE : 5,6 . Si j'ajoute le modèle lm6.fit on descend à 4 mais je préfère ne pas l'ajouter car ce modèle contient des prédicteurs non significatif.
...
data.final.train <- data.final.ts[1:168,]
data.final.test  <- data.final.ts[169:180,]
...
dataTrainFinal <- ts(data, start = c(2006,1), frequency=12)


lm3.fit <- lm(kwh ~ htdd + date + cldd + I(date^2), data= dataTrainFinal)
lm5.fit <- lm(sqrt(kwh) ~ htdd +  date + cldd  + I(date^2) , data= dataTrainFinal )
...
yEstime3 <- predict(lm3.fit, newdata = dataTestFinal)

yEstime5 <- predict(lm5.fit, newdata = dataTestFinal)
yEstime5 <- yEstime5^2
