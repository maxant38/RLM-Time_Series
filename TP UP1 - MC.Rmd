...
```{r}
acfSerie <- acf( as.vector(diffdataSeriesTwo), lag=20, ylim = c(-1,1), 
          main = expression(paste("ACF empirique de la s̩rie ", y[t])), xlab="D̩calage h", lwd=2)


pacfSerie <- pacf(
  
   as.vector(diffdataSeriesTwo), lag=20, ylim = c(-1,1), 
          main = expression(paste("PACF empirique de la s̩rie ", y[t])), xlab="D̩calage h", lwd=2
)
...
diffSerie <- diff(dataTrain)
fitBest <- auto.arima(dataTrain[,2])
#(fit <- arima(diffdataSeriesTwo, order=c(12,1,)))
summary(fitBest)
checkresiduals(fitBest)

#yEstime6 <- predict(fitBest, newdata = dataTest)
...
myforecasts <- forecast::forecast(fitBest, h=12) 
yEstime6 <- myforecasts$mean
residus6 <- yTest-yEstime6
compute_rmse(residus6)

yEstime7 <- (yEstime6 + yEstime5)/2
...
# train sarima model 
elecequip_sarima2 = auto.arima(dataSeriesTwo,
                              stepwise=FALSE,
                              approximation=FALSE,
...
# Compl̩ment : r̩gression lin̩aire
n <- length(diffdataSeriesTwo)
y <- diffdataSeriesTwo
X <- matrix(0, ncol=2, nrow=n-13)
yobs <- y[3:(n-11)] 
X[,1] <- y[2:(n-12)]
X[,2] <- y[1:(n-13)] 

reg <- lm(yobs ~ . - 1, data=data.frame(X))
summary(reg)
...
for (k in 1:11) {
  ypred.lm[k] <- mu + beta1*(y[n-11+k-1] - mu) + beta2*(y[n-11+k-2] - mu)
}
